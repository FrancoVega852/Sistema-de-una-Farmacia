...
class Venta {
    ...
    // $items = [['id'=>producto_id,'cant'=>N], ...]
    public function registrarVenta(?int $cliente_id, int $usuario_id, array $items): int {
        $this->db->begin_transaction();
        try {
            $st = $this->db->prepare("INSERT INTO Venta(cliente_id,usuario_id,total,estado) VALUES(?,?,0,'Pagada')");
            $st->bind_param("ii", $cliente_id, $usuario_id);
            $st->execute();
            ...
            foreach($items as $it){
                ...
                $pid = (int)$it['id'];  $cant = (int)$it['cant'];

                // precio + stock
                $rs = $this->db->query("SELECT precio, stock_actual FROM Producto WHERE id=".$pid);
                $p  = $rs->fetch_assoc();
                if ($p['stock_actual'] < $cant) throw new Exception("Stock insuficiente para producto $pid");

                $precio  = (float)$p['precio'];
                $sub     = $precio * $cant;
                $total  += $sub;

                $stD = $this->db->prepare("INSERT INTO DetalleVenta(venta_id,producto_id,cantidad,precio_unitario,subtotal)
                                           VALUES(?,?,?,?,?)");
                $stD->bind_param("iiidd",$venta_id,$pid,$cant,$precio,$sub);
                $stD->execute();

                // actualizar stock + historial
