...
if (!isset($_SESSION['usuario_id'])) {
    ...
    if ($isAjax) {
        ...
        echo "<div style='padding:20px;font-family:Arial'>
                ...
                <p>Inicie sesión nuevamente para registrar compras.</p>
              </div>";
    }
    exit();
}
...
if (!isset($_POST['productos']) || empty($_POST['productos'])) {
    if ($isAjax) {
        json_response([
            'ok'  => false,
            'msg' => 'Carrito vacío. No se enviaron productos para la compra.'
        ], 400);
    } else {
        exit("<div style='padding:20px;font-family:Arial'>
                <h3 style='color:#b93142'>Error: Carrito vacío</h3>
                <p>No se enviaron productos para procesar la compra.</p>
              </div>");
    }
}
...
if (empty($items)) {
    if ($isAjax) {
        json_response([
            'ok'  => false,
            'msg' => 'Carrito inválido. No se pudo interpretar los productos de la compra.'
        ], 400);
    } else {
        exit("<div style='padding:20px;font-family:Arial'>
                <h3 style='color:#b93142'>Error: Carrito inválido</h3>
                <p>No se pudo interpretar los productos de la compra.</p>
              </div>");
    }
}

/* =============================
   PROCESAR LA COMPRA (usa Venta)
   ============================= */
try {
    $conn       = new Conexion();
    $db         = $conn->conexion;
    ...
    // Guardar (mismo modelo Venta)
    $compra_id = $ventaModel->registrarVenta(
        $cliente_id,
        $_SESSION['usuario_id'],
        $items
    );
    ...
    $st->bind_param("i", $compra_id);
    $st->execute();
    $res = $st->get_result()->fetch_assoc();
    $totalCompra = (float)($res['total'] ?? 0);

    if ($totalCompra > 0) {
        $mov = $db->prepare("
            INSERT INTO Movimiento (tipo, monto, descripcion, origen, ref_id)
            VALUES ('Ingreso', ?, 'Compra cliente', 'CompraCliente', ?)
        ");
        $mov->bind_param("di", $totalCompra, $compra_id);
        $mov->execute();
    }
    ...
    if ($isAjax) {
        json_response([
            'ok'       => true,
            'venta_id' => (int)$compra_id
        ], 200);

    } else {
        header("Location: Compra_Cliente_listar.php?ok=1&id=" . $compra_id);
        exit();
    }
    ...
    if ($isAjax) {
        json_response([
            'ok'  => false,
            'msg' => 'Error al registrar la compra: ' . $msgError
        ], 500);
    } else {
        echo "<div style='padding:20px;font-family:Arial'>
                <h3 style='color:#b93142'>Error al registrar la compra</h3>
                <p>" . htmlspecialchars($msgError) . "</p>
                <a href='Compra_Cliente_listar.php' style='color:#00794f;font-weight:bold'>Volver al listado</a>
              </div>";
        exit();
    }
}
