...
<div id="nv-root">
  <!-- Toast -->
  <div id="nv-toast" class="nv-toast">Venta registrada con éxito</div>
  ...
  <div id="nv-modal" class="nv-modal-backdrop" aria-hidden="true">
    <div class="nv-modal">
      <div class="nv-modal-header">
        <div>
          <div class="nv-receipt-title"><i class="fa-solid fa-receipt"></i> Comprobante de venta</div>
      ...
      <div class="nv-modal-footer">
        ...
        <button type="button" class="nv-btn nv-btn-ghost" id="nv-rec-listado">
          <i class="fa-solid fa-list"></i> Ir al listado de ventas
        </button>
  ...
  <div class="nv-title"><i class="fa-solid fa-cash-register"></i> Nueva venta</div>
  <div class="nv-sub">Seleccioná productos del catálogo, añadilos al carrito y registrá la venta.</div>
  ...
  <div class="nv-grid">
    ...
    <div class="nv-panel">
      ...
      <div class="nv-actions" style="margin-top:14px;display:flex;gap:8px;align-items:center">
        ...
        <form action="ventas_guardar.php" method="POST" id="nv-form" style="margin-left:auto;display:flex;gap:8px;align-items:center">
...
(function(){
  ...
  const LS_KEY_Q    = 'farvec_nv_search';
  const LS_KEY_CAT  = 'farvec_nv_cat';
  const LS_KEY_STK  = 'farvec_nv_only_stock';
  const LS_KEY_REC  = 'farvec_nv_only_receta';
  const LS_KEY_PAGE = 'farvec_nv_page';
  ...
  if(btnRecNueva){
    btnRecNueva.addEventListener('click',()=>{
      closeModal();
      nvVaciar();
      showToast('Lista para una nueva venta');
    });
  }
  ...
  if(btnRecListado){
    btnRecListado.addEventListener('click',()=>{
      closeModal();
      const link = document.querySelector('.sidebar a[href="ventas_listar.php"]');
      if(link) link.click();
      else window.location.href = 'ventas_listar.php';
    });
  }
  ...
  if(btnRecMenu){
    btnRecMenu.addEventListener('click',()=>{
      closeModal();
      const linkHome = document.querySelector('.top-actions a[href="Menu.php"]');
      if(linkHome) linkHome.click();
      else window.location.href = 'Menu.php';
    });
  }

  /* ============ ENVÍO AJAX DE LA VENTA ============ */
  form.addEventListener('submit', async (ev)=>{
    ...
    btnReg.disabled = true;
    showToast('Registrando venta…');

    try{
      const fd = new FormData(form);
      const res = await fetch('ventas_guardar.php', {
        method:'POST',
        body: fd,
      ...
      if(!res.ok || (data && data.ok === false)){
        throw new Error(data && data.msg ? data.msg : 'No se pudo registrar la venta.');
      }

      // Venta OK → armar recibo
      const ventaId = data && data.venta_id ? data.venta_id : '—';
      const ahora = new Date();
      const fechaStr = ahora.toLocaleString('es-AR');

      if(recInfo){
        recInfo.textContent = `Venta #${ventaId} · ${fechaStr} · Cliente: ${clienteNombre}`;
      }
      if(recBody){
        recBody.innerHTML = '';
      ...
      alert('Error al registrar la venta: ' + (err.message || 'desconocido'));
    }finally{
