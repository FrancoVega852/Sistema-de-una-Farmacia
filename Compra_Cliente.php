...
class Compra_Cliente {
     ...
     */
    public function listarCompras(): mysqli_result {
    ...
    /**
     * Registrar nueva compra (venta asociada al cliente)
     * @param int|null $cliente_id  ID cliente (null = consumidor final)
     * @param int $usuario_id       ID usuario logueado
     * @param array $items          Productos comprados [['id'=>producto_id,'cant'=>N], ...]
     * @return int ID de la compra registrada
     */
    public function registrarCompra(?int $cliente_id, int $usuario_id, array $items): int {
        $this->db->begin_transaction();
        try {
            // Insert inicial de la compra (venta) con total en 0
            ...
            foreach($items as $it){
                ...
                // Insertar detalle de la compra (venta)
                $stD = $this->db->prepare("INSERT INTO DetalleVenta
                          (venta_id, producto_id, cantidad, precio_unitario, subtotal)
                          VALUES (?, ?, ?, ?, ?)");
                $stD->bind_param("iiidd", $compra_id, $pid, $cant, $precio, $sub);
                $stD->execute();
            ...
            $stU->bind_param("di", $total, $compra_id);
            $stU->execute();
